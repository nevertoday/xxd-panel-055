...
上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 055 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。
...
写真の中で最も物語性のある主体、動作、空間関係を見極め、それを支えない環境情報を削ります。単なるパステルフィルターではなく、識別可能な現実の手掛かりを、主従と読解経路のある絵画的構成へ組み直します。

```text
人物・輪郭・姿勢・関係を固定 → 記憶に残る物語の手掛かりを選ぶ → 複雑な場面を圧縮 → 焦点を一つ作る → 柔らかなパステル面と淡い油彩で再構成 → 空気感のある青と淡い中性色を保つ → 最小限の編集文字を統合
```

## 完成作品で見分けやすいスタイル特性

- 主体の同一性、重要な姿勢、物語関係が読め、背景全体は描き写しません。
- ガッシュ、アクリル、パステル、控えめなペインティングナイフ跡が、柔らかく明るい手仕事の表面を作ります。
- 淡い粉青、霧青、空色、象牙、クリーム、薄いベージュが主調で、くすみピンクや柔らかな桃色は小さな差し色です。
- 偏心、拡大、部分的な裁ち落としと静かな余白で、明確な読解経路を作ります。
- 文字は小さく軽い編集注記として、現在の画像の内容や暗示からのみ生まれます。

美的制約と拒否項目は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあり、Skill とランタイムアダプターは今回の出力変数だけを扱います。 [Skill](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-055-prompt.en.md)
