<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 055 project banner" width="1200"></p>
...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091726763701555207"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 055 sample 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091726763701555207"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 055 sample 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091726763701555207"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 055 sample 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091726763701555207"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 055 sample 4"></a></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 055 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 055 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 055 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 055 Sample 8"></td>
  </tr>
</table>

## Additional 3:4 Top–Bottom Samples

These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 055's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 055 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 055 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 055 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 055 additional top-bottom sample 4"></td>
  </tr>
</table>
...
**Panel 055** is suited to work that must retain the source's essential content while pursuing this visual direction: **Turn light, gesture, and spatial relation into a gentle visual narrative**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 055 on this image and recommend the best layout and size first.
...
|---|---|
| **What you will get** | Turn light, gesture, and spatial relation into a gentle visual narrative |
| **Recognizable signature** | SUBJECT NARRATIVE · HEALING PASTELS · LIGHT OIL TEXTURE · AIRY BLUE · EDITORIAL SPACE |
...
The style first identifies the source's most narrative subject, gesture, and spatial relation, then removes environmental detail that does not support them. It is not a pastel filter: recognizable reality is reorganized into a painted composition with hierarchy and a deliberate reading path.

```text
lock identity, silhouette, posture, and relation → choose the memorable narrative cue → compress the scene → establish one clear focal centre → rebuild with soft pastel masses and light oil texture → hold airy blue and pale neutrals → integrate minimal editorial text
```

## What makes the finished style recognizable

- Subject identity, key posture, and narrative relation remain legible without reproducing the whole background.
- Gouache, acrylic, pastel, and restrained palette-knife marks create a soft, bright handmade surface.
- Powder blue, mist blue, sky blue, ivory, cream, and pale beige establish the atmosphere; muted blush or soft peach appears only as an accent.
- Off-centre placement, enlargement, or selective cropping creates a clear reading path through depth, direction, and quiet space.
- Type stays small, light, and editorial, and grows only from the current image's content or implication.

Aesthetic constraints and rejection rules live only in the [original source brief](references/original-prompt/zh-CN.md); the Skill and runtime adapter handle delivery variables only. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-055-prompt.en.md)
...
```text
/xxd-panel-055 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-055 photo.jpg --prefs last
/xxd-panel-055 photo.jpg --prefs edit
/xxd-panel-055 photo.jpg --prefs new
```
...
/xxd-panel-055 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-055 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-055 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-055 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-055 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-055 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-055 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-055 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-055 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-055 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-055 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 055 to a whole directory; shared settings resolve once
/xxd-panel-055 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-055 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-055.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-055" ~/.codex/skills/xxd-panel-055
```

Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-055`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-055-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-055-prompt.zh-CN.md)
